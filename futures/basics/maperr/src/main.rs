...
fn main() {
    ...
    // NOTES: 
    // 1. MapErr executes the closures only when future results err or else passes the result along unaltered
    ...
    assert_eq!(Err(true), reactor.run(f)); // MapErr poll will wrap 'true' to Err(true)
